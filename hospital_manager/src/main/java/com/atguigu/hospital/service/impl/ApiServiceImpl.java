...
public class ApiServiceImpl implements ApiService {
    ...
    @Override
    public boolean saveHospital(String da) {
        ...
        JSONObject jsonObject = JSONObject.parseObject(data.toString());//将string对象转换为json对象

        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hoscode",this.getHoscode()); //根据id=1的主键，查询出来的医院编号
        ...
        paramMap.put("logoData", jsonObject.getString("logoData"));//图片

        JSONObject bookingRule = jsonObject.getJSONObject("bookingRule");
        paramMap.put("bookingRule",bookingRule.toJSONString());//实体对象转换成Json字符串

        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());//获取时间戳
        paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));//根据id=1的主键,获取签名
